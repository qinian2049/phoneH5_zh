        ...
        layer.open({
          ...
          btn: [getI18nPageByKey("cancel"), getI18nPageByKey("okBtn")],btn2:function(){//
          	  var stateInfo=$("input:checked").parent().siblings().text();
           		Datas({
           			url:"order_cancel",
           			datas:{"id":id,"user_id":$.session.get("userId"),"other_state_info":"","state_info":stateInfo},
          ...
          btn: [getI18nPageByKey("cancel"), getI18nPageByKey("okBtn")],btn2:function(){//
           var val= $(".rease").find("textarea").val();
           		Datas({
           			...
           			datas:{"of_id":id,"user_id":$.session.get('userId'),"content":val},
           			success:function(data){
                  // console.log(data);
           				if (data.status) {
           					// layer.msg(data.message);
...
function pay(lineTypes,totalPrice,orderId){
  ...
  if (lineTypes==true) {
     lineType=1;
    window.location.href="guestPayBill.html?lineType="+lineType+"&totalPrice="+totalPrice+"&orderId="+orderId;
  }else{
     lineType=0;
     window.location.href="guestPayBill.html?lineType="+lineType+"&totalPrice="+totalPrice+"&orderId="+orderId;
  }
...
function menuDetai(id,storeId,orderStatus){
  window.location.href=" orderDetailWaitGoods.html?order_id="+id+"&store_id="+storeId+"&orderStatus="+orderStatus;
...
function Item(goodId){
  window.location.href="product.html?goods_id="+goodId;
}
//已收货更多
...
function recode_detail(id){
  window.location.href="returnmoneyAgreen.html?order_id="+id;
}
//去评价
function Evaluate(id,store_id){
  window.location.href="publicComment.html?order_id="+id+"&store_id="+store_id;
}
function complain(id,storeId){
   window.location.href="chooseSubmitPro.html?order_id="+id+"&storeId="+storeId;
}
